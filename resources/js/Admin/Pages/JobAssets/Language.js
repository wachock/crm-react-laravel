  ...
  const headers = {
    ...
    const getLanguages = () => {
        axios.get("/api/admin/languages", { headers }).then((response) => {
            if (response.data.languages.data.length > 0) {
                setLanguage(response.data.languages.data);
                setPageCount(response.data.languages.last_page);
            } else {
    ...
    const handlePageClick = async (data) => {
        ...
        axios
            .get("/api/admin/languages?page=" + currentPage, { headers })
            .then((response) => {
                if (response.data.languages.data.length > 0) {
                    setLanguage(response.data.languages.data);
                    setPageCount(response.data.languages.last_page);
    ...
    const getFilteredLanguage = (response) => {
        if (response.data.languages.data.length > 0) {
            setLanguage(response.data.languages.data);
            setPageCount(response.data.languages.last_page);
        } else {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            confirmButtonText: "Yes, Delete Language!",
        }).then((result) => {
            if (result.isConfirmed) {
                axios
                    .delete(`/api/admin/languages/${id}`, { headers })
                    .then((response) => {
                        Swal.fire(
                            "Deleted!",
                ...
                <div className='row'>
                    ...
                    <div className='col-sm-6'>
                        <div className='text-right'>                          
                            <Link to='/admin/add-language' className='btn btn-success addButton'>Add Language</Link>
                        </div>
                    </div>
                </div>
            </div>
            <LanguageFilter getFilteredLanguage={getFilteredLanguage}/>
                    ...
                    <table className="table bg-white table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th className='text-center bg-dark-grey text-white'>Language</th>
                                ...
                                <tr key={index}>
                                    ...
                                    <td className='text-center'>
                                        <div className="text-center">
                                            <Link
                                                to={`/admin/edit-language/${item.id}`}
